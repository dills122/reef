-- Split durable command payloads from the hot command metadata row.
-- command_log.commands remains the durable accepted-command ledger, while
-- command_log.command_payloads stores the full request JSON needed for replay.

CREATE TABLE IF NOT EXISTS command_log.command_payloads (
  command_id TEXT PRIMARY KEY REFERENCES command_log.commands(command_id) ON DELETE CASCADE,
  payload_json JSONB NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

INSERT INTO command_log.command_payloads(command_id, payload_json, created_at)
SELECT command_id, payload_json, created_at
FROM command_log.commands
WHERE payload_json <> '{}'::jsonb
ON CONFLICT (command_id) DO NOTHING;

DROP FUNCTION IF EXISTS command_log.command_append(
  TEXT,
  TEXT,
  TEXT,
  TEXT,
...
BEGIN
  ...
  FROM command_log.commands c
  LEFT JOIN command_log.command_payloads p ON p.command_id = c.command_id
