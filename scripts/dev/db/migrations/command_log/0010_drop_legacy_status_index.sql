-- The active queue and terminal result tables now own command lifecycle lookups.
-- Keep legacy command status defaults for compatibility, but remove the old
-- command-table status index from the hot append path.

DROP INDEX IF EXISTS command_log.idx_command_log_commands_status_received;
