-- Complete indexed market recomputation:0053 still scanned lifecycle state for
-- side-independent presence/MAX(currency). Keep those semantics with a bounded
-- lookup on a matching partial index. Price/quantity and claim logic unchanged.

CREATE INDEX IF NOT EXISTS idx_order_lifecycle_state_book_currency
  ON runtime.order_lifecycle_state(instrument_id, currency DESC NULLS LAST)
