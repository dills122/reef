-- Match incremental lifecycle business state to the full rebuild: terminal
-- orders have zero remaining quantity in both text and numeric columns, and
-- zero original/current quantity alone does not imply an execution-filled order.
-- No canonical facts, queue protocol, batch limits, or scheduling change.
-- Existing projections require explicit recovery after deployment: re-dirty
-- affected lifecycle orders (then drain lifecycle and market queues), or run
-- full lifecycle rebuild followed by full market refresh with writers stopped.
-- Rollout is incomplete until existing-state recovery and verification finish:
-- quiesce canonical writers and both maintainers; apply migration; invoke full
-- rebuildOrderLifecycleState(), then refreshMarketDataSnapshots() with verified
-- committed source metadata; compare business rows excluding only updated_at;
-- resume writers/maintainers only after equality and empty-queue checks pass.
-- A function replacement alone does not repair rows behind already empty queues.
-- This migration intentionally does not scan/backfill existing business rows;
-- operators must schedule recovery explicitly, outside measured service work.
