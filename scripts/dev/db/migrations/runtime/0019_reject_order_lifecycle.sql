-- Reflect rejected SubmitOrder commands as REJECTED orders instead of leaving them
-- invisible in runtime.orders / runtime.order_lifecycle_state. Boundary-level rejects
-- (authorization, missing reference data) never reach a real order attempt and stay
-- excluded; only genuine engine-level rejects get an order row.
      ...
      jsonb_build_object(
        ...
        'acceptedOrder', CASE
          ...
          THEN jsonb_build_object(
            'orderId', order_id,
            'engineOrderId', CASE WHEN result_status = 'rejected' THEN '' ELSE COALESCE(result_payload #>> '{accepted,engineOrderId}', '') END,
