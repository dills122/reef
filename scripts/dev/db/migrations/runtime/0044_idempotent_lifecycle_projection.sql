-- Avoid rewrite amplification when a dirty order recomputes to the same
-- lifecycle state. The projector still clears and counts selected dirty rows
-- as progress, but only changed lifecycle rows fan out to market-data dirties.
