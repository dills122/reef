-- Prefer order projection facts carried by the durable venue-event-batch result payload.
-- command_log.command_payloads stays as a compatibility fallback for older batches and
-- command-log-backed runs, but no-DB direct consume can now project full order rows.
