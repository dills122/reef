-- Bound market recomputation to indexed best-price lookups and aggregation of
-- that price level, using existing0028 bid/ask partial indexes. Preserve the
-- side-independent presence/currency aggregate across all eligible limit orders.
-- Claim locks, fresh VOLATILE statement snapshot, dirty deletion, metadata and
-- all snapshot business fields remain unchanged from0052. Keep MAX/MIN on the
-- chosen level as well as SUM so equal-price NUMERIC representations retain
-- aggregate behavior instead of taking the LIMIT1 row as output representation. No index/settings or
-- existing-state backfill changes; source facts remain authoritative.
...
BEGIN
  ...
  present_instruments AS (
    -- Preserve side-independent presence and MAX(currency), including non-BUY/SELL
    -- values if present. Do not assume single currency or take currency from best row.
