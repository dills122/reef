...
cmd_plan_goal() {
  ...
  printf '  projection_downstream_instrumentation_enabled=%s\n' "${PROJECTION_DOWNSTREAM_INSTRUMENTATION_ENABLED:-false}"
  printf '  require_projection_cohort_authority=%s\n' "${REEF_DO_REQUIRE_PROJECTION_COHORT_AUTHORITY:-0}"
  printf '  require_sustained_downstream_freshness=%s\n' "${REEF_DO_REQUIRE_SUSTAINED_DOWNSTREAM_FRESHNESS:-0}"
...
cmd_run() {
  ...
  REEF_DO_REQUIRE_PROJECTION_COHORT_AUTHORITY="${REEF_DO_REQUIRE_PROJECTION_COHORT_AUTHORITY:-}" \
