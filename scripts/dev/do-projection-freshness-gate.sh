...
gates:
  ...
  projector DB retry delta = 0
  projector final lag = 0
  direct-stream active partitions >= 16
  direct-stream partition skew <= 4
  DB WAL/activity/settings/pg_stat_io/pg_stat_statements diagnostics present
  exact upstream and post-commit downstream cohort authority
USAGE
}
...
export REEF_DO_MAX_PROJECTION_DB_RETRIES="${REEF_DO_MAX_PROJECTION_DB_RETRIES:-0}"
export REEF_DO_PROJECTION_STAGE="${REEF_DO_PROJECTION_STAGE:-full}"
...
export REEF_DO_REQUIRE_PG_STAT_STATEMENTS="${REEF_DO_REQUIRE_PG_STAT_STATEMENTS:-1}"
export PROJECTION_DOWNSTREAM_INSTRUMENTATION_ENABLED="${PROJECTION_DOWNSTREAM_INSTRUMENTATION_ENABLED:-true}"
export REEF_DO_REQUIRE_PROJECTION_COHORT_AUTHORITY="${REEF_DO_REQUIRE_PROJECTION_COHORT_AUTHORITY:-1}"
...
export REEF_DO_REQUIRED_ORDER_LIFECYCLE_MAINTAINERS="${REEF_DO_REQUIRED_ORDER_LIFECYCLE_MAINTAINERS:-1}"
export REEF_DO_REQUIRED_MARKET_DATA_MAINTAINERS="${REEF_DO_REQUIRED_MARKET_DATA_MAINTAINERS:-1}"
