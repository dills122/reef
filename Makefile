...
.PHONY: test test-go test-platform-runtime test-simulator fmt-go check-proto-additive bench-matching-engine bench-matching-engine-check bench-platform-runtime-check dev-up dev-down dev-reset dev-db-migrate dev-smoke dev-stress dev-stress-diagnostics dev-intake-bench dev-command-log-prune dev-admin dev-sim dev-replay dev-throughput-campaign dev-throughput-compare
