...
dev-smoke-bot-arena-local:
	@$(MAKE) check-js-runtime JS_RUNTIME=$(JS_RUNTIME)
	$(JS_RUNTIME) scripts/dev/arena-local-tick-run.mjs --submit-mode=live --venue-url=$(or $(VENUE_URL),http://127.0.0.1:8080) --seed-reference --compartment=ses --out=$(or $(OUT),/tmp/reef-arena-local-tick-run-live.json) $(ARGS)

dev-smoke-bot-arena-local-persist:
	@$(MAKE) check-js-runtime JS_RUNTIME=$(JS_RUNTIME)
	$(JS_RUNTIME) scripts/dev/arena-local-tick-run.mjs --submit-mode=live --venue-url=$(or $(VENUE_URL),http://127.0.0.1:8080) --arena-admin-url=$(or $(ARENA_ADMIN_URL),$(or $(VENUE_URL),http://127.0.0.1:8080)) --seed-reference --persist-results --compartment=ses --out=$(or $(OUT),/tmp/reef-arena-local-tick-run-persist.json) $(ARGS)
