...
It performs the hosted path: build a hosted artifact from the bot source, allow only approved SDK/package imports, scan source and final artifact, load the artifact through SES, run the deterministic simulated market fixture, execute strategy hooks and tick hooks, enforce configured data-call/order-action/trade-command limits, and produce an `approved_for_merge` or `do_not_merge` report. This is the first PR gate for bot authors — deterministic and fixture-backed so failures can be replayed.

The tester marks a bot `do_not_merge` for: sandbox scanner failures, unapproved imports or dynamic module loading, hosted build/load failures, lifecycle or tick timeout, per-tick order action limit violations, trade-command-per-second limit violations, data-call rate limit violations, unsupported order action types, venue adapter denials, or hosted worker wall-time/output-limit failures. The harness covers these categories at the SDK layer except true production CPU/memory pressure, which belongs to the hosted worker/container runtime.
...
- Arena run registration requires canonical `policyEnvelopeHash`, scoring-policy
  version/hash, and economic-policy version/hash fields. Result ingestion must
  repeat the accepted scoring and envelope hashes, may update a result only
  while the run is `running`, and becomes immutable after score publication.
- `arena.run_bot_results` stores bot/run scoring facts outside the trading hot path. `GET /admin/v1/arena/run-bot-results?runId=...` exposes raw persisted scoring facts, while `GET /admin/v1/arena/leaderboard?modeId=...&scoringPolicyVersion=...` exposes only results whose version and hashes match the completed run lock, ranked by final equity, realized PnL, drawdown, run ID, and bot ID.
- `scripts/dev/arena-ingest-bot-run-result.mjs` maps hosted simulation/test-bot summary counters into `arena.run_bot_results`; final equity, realized PnL, and drawdown remain explicit ingestion inputs and the summary or CLI must provide canonical scoring-policy and policy-envelope hashes.
