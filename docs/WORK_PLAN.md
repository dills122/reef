...
- first deterministic lifecycle scenarios have promoted local live assertion reports, but are not locked end to end; P1 now has a promoted same-run direct-stream report combining run-scoped command status, exact replay counters, native hidden-depth timeline proof, and replay-folded visibility proof, but its retained-stream data-availability snapshot records projection lag and is not a zero-lag claim; P2 now has promoted direct-stream replay/checksum evidence and a settled `instant-post-trade-v1` chain proof, with the remaining decision limited to whether direct-stream lock requires the public settlement facts read instead of the accepted local settlement-facts append artifact
...
6. Lock first lifecycle scenarios.
   ...
   - Remaining lock work: decide whether P1's recorded projection lag is acceptable for lock or rerun on a globally drained stack with `--require-zero-projection-lag`; attach P2 replay/checksum for the exception chain; promote a P2 live settled instant-post-trade chain report when allocation/confirmation/affirmation/instruction/attempt/settled facts are present.
