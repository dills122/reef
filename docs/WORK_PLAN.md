...
4. Complete venue lifecycle projection.
   ...
   - Deterministic timeline sequencing is implemented for canonical venue-event projection payloads carrying `streamSequence`: `runtime_events.sequence_number` is derived from stream sequence plus event ordinal, and the old `runtime.runtime_trace_sequences` allocator is only used for legacy payloads without stream sequence metadata. The short `5k` full-projection checkpoint is green. Dirty-queue WAL reduction is implemented locally by making `order_lifecycle_dirty` and `market_data_snapshot_dirty` unlogged rebuildable queues and avoiding redundant dirty timestamp updates. The next work is rerunning the `5k` full projection shape to quantify WAL/dirty-table growth delta, then reducing `runtime_events` and lifecycle/fill write amplification before longer `5k` soaks or higher-rate projection gates.
