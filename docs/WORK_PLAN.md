...
3. Complete venue lifecycle projection.
   ...
   - `GET /api/v1/data/availability` now inventories bot/user read surfaces with source, freshness model, projection lag, and watermarks, so simulator reports can distinguish durable fact reads from projection-freshness claims.
   - `packages/bot-sdk/src/live-client.ts` is the first Bot SDK client wiring `marketData`/`historical`/`orders.current()`/`history()` plus data availability to real HTTP instead of fixtures; the venue adapter converts bot dollar-denominated `limitPrice` values to fixed-point nanos before submit/modify commands. Not yet wired into the tick-runner (`runner.ts`/`strategy-runner.ts`, still fixture-only).
