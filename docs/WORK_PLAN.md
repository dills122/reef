...
3. Complete venue lifecycle projection.
   ...
   - `packages/bot-sdk/src/live-client.ts` wires `marketData`/`historical`/`orders.current()`/`history()` plus data availability to real HTTP instead of fixtures; `runner.ts` and `strategy-runner.ts` accept those clients through opt-in `readClients` while fixture mode remains the default. The venue adapter converts bot dollar-denominated `limitPrice` values to fixed-point nanos before submit/modify commands.
