...
The direct materializer portion of this checkpoint has local crash/replay proof and DigitalOcean `10k` `soak-5m` promotion evidence. The first named short projection/read-model freshness gate is also green: local durable proof passed, and DigitalOcean run `do-benchmark-20260716T005756Z` proved `150,001` accepted/direct-acked/materialized/projected commands at `2499.52/sec` with projection lag `0`, accepted/materialized gap `0`, materialized/projected gap `0`, p95 `47.31ms`, p99 `159.32ms`, and `100%` success. July 17 pressure evidence found the current projection knee: `5k` accepted/direct-acked/materialized holds on c-16, but full projection freshness ends with nonzero watermark lag even after removing the observed deadlock mode. The active engineering priority is API/control-plane hardening plus the projection scaling sequence in [`PROJECTION_THROUGHPUT_SCALING_PLAN.md`](./PROJECTION_THROUGHPUT_SCALING_PLAN.md); do not redefine venue-core command acceptance as UI/control-room freshness.
...
3. Maintain and extend named crash/restart tests for the direct durable path.
   ...
   - first short projection/read-model freshness gate is closed; remaining performance follow-up is scaling projection evidence through the `5k` freshness knee before targeting the `10k` materializer baseline.
...
- durable hot-ingress plus canonical materialization now has a clean `10k` `soak-5m` baseline; the short projection/read-model freshness gate is green at `2.5k rps`, and the remaining throughput gap is scaling projection freshness toward the same durable venue-event load as the materializer baseline
...
4. Complete venue lifecycle projection.
   - This promotion track is now green for the short `2.5k rps` gate. Keep projected throughput, projection lag/watermarks, replay idempotency, and read API freshness reported separately from canonical accepted/materialized throughput.
   - Remote gate command: `make do-projection-freshness-gate ARGS=plan|run-destroy`. It runs the Redpanda direct-stream plus venue-event-materializer path on DigitalOcean with venue-event-batch projection, order-lifecycle projection, and market-data projection enabled, then fails if projected work does not catch up to durable canonical materialization with zero materialized/projected gap, zero projector lag, zero projector DB retry delta, and zero projection DB deadlocks after the configured drain window.
   ...
   - 2026-07-16 named short projection gate evidence: DO run `do-benchmark-20260716T005756Z` passed at `2.5k rps` on c-16 with `150,001` attempted/accepted/direct-acked/materialized/projected commands, projected rps `2499.52`, p95 `47.31ms`, p99 `159.32ms`, projection lag `0`, accepted/materialized gap `0`, materialized/projected gap `0`, and `100%` success. Local artifacts live under `reports/do-benchmark/do-benchmark-20260716T005756Z/`.
   - 2026-07-17 projection pressure evidence: DO run `do-benchmark-20260717T014839Z` reconfirmed the corrected `2.5k` gate with `149,975` attempted/accepted/direct-acked/materialized/projected, lag `0`, gaps `0`, p95 `31.31ms`, and p99 `79.38ms`. The `5k` run `do-benchmark-20260717T015658Z` proved venue-core held `299,950` accepted/direct-acked/materialized but failed projection freshness with a `1,834` materialized/projected gap, one projector failure, and one projection-postgres deadlock in `runtime.runtime_persist_submit_outcomes`. After ordering `runtime_events` inserts by `event_id`, `do-benchmark-20260717T020807Z` removed the deadlock and count gap (`299,952` accepted/direct-acked/materialized/projected, deadlocks `0`) but still ended with projection watermark lag `1,367`. After the stage split and Docker Compose env pass-through fix, `do-benchmark-20260717T131344Z` passed the `5k` `command-status` gate with `299,955` attempted/accepted/direct-acked/materialized/projected, lag `0`, gaps `0`, p95 `74.49ms`, p99 `112.93ms`, and projector failures/retries/deadlocks `0`. After deterministic timeline sequencing, `do-benchmark-20260717T134058Z` passed the `5k` full-projection gate with `299,804` attempted/accepted/direct-acked/materialized/projected, lag `0`, gaps `0`, p95 `71.89ms`, p99 `112.89ms`, and projector failures/retries/deadlocks `0`. The current fix ladder is [`PROJECTION_THROUGHPUT_SCALING_PLAN.md`](./PROJECTION_THROUGHPUT_SCALING_PLAN.md).
   - Projection stage split has started and has remote evidence: `runtime.runtime_persist_submit_outcomes` now has command-status and timeline stage wrappers, `STREAM_ACK_PROJECTION_STAGE=command-status` / `REEF_DO_PROJECTION_STAGE=command-status` can measure command-status/lifecycle freshness without requiring runtime event/timeline rows to be current in the same gate, `STREAM_ACK_PROJECTION_STAGE=timeline` / `REEF_DO_PROJECTION_STAGE=timeline` can isolate runtime event/trace projection cost, and the default remains `full`.
   - Deterministic timeline sequencing is implemented for canonical venue-event projection payloads carrying `streamSequence`: `runtime_events.sequence_number` is derived from stream sequence plus event ordinal, and the old `runtime.runtime_trace_sequences` allocator is only used for legacy payloads without stream sequence metadata. The short `5k` full-projection checkpoint `do-benchmark-20260717T134058Z` is green. Dirty-queue WAL reduction is implemented by making `order_lifecycle_dirty` and `market_data_snapshot_dirty` unlogged rebuildable queues and avoiding redundant dirty timestamp updates, but the remote A/B `do-benchmark-20260717T142157Z` did not preserve freshness: projected `252,866` of `299,954` with final lag/gap `47,088`. Keep it as diagnostic evidence, not promotion evidence. The next work is reducing `runtime_events` and lifecycle/fill write amplification before longer `5k` soaks or higher-rate projection gates.
...
- [`COMMAND_INTAKE_PROCESS.md`](./COMMAND_INTAKE_PROCESS.md)
- [`API_SURFACE_POLICY.md`](./API_SURFACE_POLICY.md)
- [`ARCHITECTURE_THROUGHPUT_TRACKER.md`](./archive/ARCHITECTURE_THROUGHPUT_TRACKER.md)
- [`DIGITALOCEAN_STRESS_TEST_PLAN.md`](./DIGITALOCEAN_STRESS_TEST_PLAN.md)
...
- `archive/SPRINT_CRITICAL_QUALITY_HARDENING.md`
- `archive/SPRINT_POST_MATCH_ENGINES.md`
- `archive/BOT_ARENA_STRESS_BASELINE_2026-07-01.md`
- May 2026 throughput and abuse-breaker baseline reports
- `archive/THROUGHPUT_SCALING_WORK_PLAN.md` — P2 (stream-ack ingress), P3 (stream idempotency/partition workers), and P5.5 (engine shards/hot book) are superseded by D-041 and the July 2026 stream-ack sunset and hot-book checkpoints (see `DECISIONS.md` and `PERFORMANCE_LEARNINGS.md`); it is still cited elsewhere for the `7500`/`10000` completed-commands/sec per-instance target, which remains valid
- `archive/ARCHITECTURE_THROUGHPUT_PLAN.md`, `archive/ARCHITECTURE_THROUGHPUT_TRACKER.md`, `archive/STREAM_ACK_ARCHITECTURE_PLAN.md` — narrate the pre-D-041 stream-ack-centric throughput story; superseded by the D-041 Kafka-direct-consumption direction (see `DECISIONS.md`). Retained as evidence of the evolution, not as the current design.
