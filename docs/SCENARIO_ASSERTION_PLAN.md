...
On 2026-07-14, P1 passed locally on a `sync-result` stack started with `ORDER_LIFECYCLE_PROJECTOR_ENABLED=true` and `MARKET_DATA_PROJECTOR_ENABLED=true`; report: `reports/scenario-assertions/p1-golden-hidden-cross-live-20260714.json`. The report passed 25 assertions and proves the P1 scenario trades by deterministic trade ids even when the instrument-level `XYZ` public tape also contains an unrelated P2 trade. A later direct-stream replay attempt exposed a lock gap: `/api/v1/commands/{commandId}` can return a prior canonical outcome when the same deterministic command ids exist in retained streams, so replay promotion must use `--run-scoped-command-ids` or otherwise prove command-status evidence is scoped to the current scenario run/event stream. The 2026-07-14 scoped direct-stream replay attempt produced a clean exact replay-check artifact for three P1 commands at `reports/scenario-assertions/p1-golden-hidden-cross-replay-check-20260714.json`; `reports/scenario-assertions/p1-golden-hidden-cross-replay-live-20260714.json` attaches that replay checksum to the earlier passing live report for evidence review. P1 is not locked until a same-run report is promoted with run-scoped command status, clean exact replay counters, native `visibilityTimeline` proof, and acceptable projection-lag evidence for the selected stack.

On 2026-07-14, P2 command smoke passed locally; report: `reports/scenario-assertions/p2-settlement-break-repair-live-20260714-command-smoke.json`. P2 assertion also passed locally after seeding facts with `make dev-seed-p2-settlement-facts SCENARIO_RUN_ID=p2-settlement-break-repair-live-20260714`; report: `reports/scenario-assertions/p2-settlement-break-repair-live-20260714.json`. The report passed 18 assertions for the exception chain. It does not contain the settled instant-post-trade fact chain; that remains a separate live proof when allocation/confirmation/affirmation/instruction/attempt/settled facts are present.
