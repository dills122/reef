...
- `/api/v1/market-data/bars/{instrumentId}` exposes intraday OHLCV bars (`interval` one of `1m`/`5m`/`15m`/`1h`, `start`/`end` bounds) aggregated from `runtime.trades` via Postgres `date_bin`, matching the Bot SDK's `BotHistoricalBarsRequestV1`/`HistoricalBarV1` contract exactly. `/api/v1/orders/current` and `/api/v1/orders/history` expose participant-scoped own-order reads (joining `runtime.orders` and `runtime.order_lifecycle_state`), closing the previous "no scoped orders query" gap — the only prior order read was an unscoped all-participants dump. `/api/v1/data/availability` now inventories these bot/user data surfaces with endpoint, source, freshness model, and projection lag/watermark where available, so tests can state whether they prove durable facts, projection freshness, or only command completion.
- `packages/bot-sdk/src/live-client.ts` adds the first live-read wiring in the Bot SDK: `createLiveMarketDataClientV1`, `createLiveHistoricalDataClientV1`, `createLiveOwnOrdersReadClientV1`, and `createLiveBotContextV1` call the endpoints above over real HTTP. Previously every `ctx.marketData`/`ctx.historical`/`ctx.orders.current()`/`history()` call was fixture-backed even in "hosted"/"live" runs; only order actions (submit/modify/cancel) reached a real venue via `venue-adapter.ts`. Not yet wired into `runner.ts`/`strategy-runner.ts`'s tick loop (still fixture-only) — usable directly by scripts/operators until that runner integration is a deliberate follow-up. Prices are stored venue-wide as fixed-point nanos (`contracts/proto/order_execution.proto`'s `Price.nanos`, `price_nanos = price_dollars * 1e9`); the live client's reads now divide by that scale before returning `number` fields to bot code, matching the dollar-denominated convention shown in `docs/BOT_SDK_AUTHOR_GUIDE.md`. `venue-adapter.ts`'s write path (`placeLimit`/`modify`) does **not** do the inverse conversion — it sends a bot's dollar `limitPrice` straight through as nanos, which is a separate, pre-existing bug (not fixed here) that would corrupt any real order submission with a non-integer-nanos price.
...
5. Order-lifecycle-state and market-data top-of-book snapshot maintenance are both now incremental (dirty-tracked, not full-table rebuild). Public trade tape and intraday bars are now live (`/api/v1/market-data/trades/{instrumentId}`, `/api/v1/market-data/bars/{instrumentId}`), and the Bot SDK has its first live-read client (`packages/bot-sdk/src/live-client.ts`) calling them, plus participant-scoped own-order reads (`/api/v1/orders/current`, `/api/v1/orders/history`). Depth reads (`/api/v1/market-data/depth/{instrumentId}`) still aggregate remaining open lifecycle quantity at request time rather than from a maintained projection; add venue-session-specific depth on top of the current lifecycle-state snapshot next. Wiring the live client into `runner.ts`/`strategy-runner.ts`'s tick loop (still fixture-only) remains a deliberate follow-up, not done here.
