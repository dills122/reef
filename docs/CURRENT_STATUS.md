...
- The named local durable-path crash gate has a first harness: `make dev-smoke-venue-event-crash-gate` drives `scripts/dev/venue-event-crash-gate.mjs` across engine, materializer, projector, and replay/check failure points. Treat it as active hardening work until the named cases are stable enough to promote.
- The DigitalOcean benchmark path is no longer only a plan: `infra/do-benchmark`, `scripts/dev/do-benchmark-host.sh`, `make do-benchmark`, and `scripts/dev/do-benchmark-check.mjs` provide the bridge OpenTofu/host-control/report-validation harness. The dedicated `infra/simulation-runner` run-plane remains the target replacement for that bridge.
