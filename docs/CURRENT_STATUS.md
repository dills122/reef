...
5. Order-lifecycle-state and market-data top-of-book snapshot maintenance are both now incremental (dirty-tracked, not full-table rebuild). Public trade tape (`/api/v1/market-data/trades/{instrumentId}`) is also now live, reading durable `runtime.trades` directly. Depth reads (`/api/v1/market-data/depth/{instrumentId}`) still aggregate remaining open lifecycle quantity at request time rather than from a maintained projection; add venue-session-specific depth on top of the current lifecycle-state snapshot next, then intraday bars/aggregates (the Bot SDK's `intradayBars()`/`intradayBarsBatch()` are fixture-only today, no live backend).
