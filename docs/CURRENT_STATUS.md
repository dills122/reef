...
- DigitalOcean 2026-07-12 short projection evidence shows the projection path can catch up cleanly at `2.5k rps` on c-16 after splitting four projector instances across active direct-stream partitions `0-15`: run `do-benchmark-20260712T172412Z` accepted/materialized/projected `149,976` commands, projected rps `2499.56`, p95 `35.98ms`, p99 `82.06ms`, projection lag `0`, materialized/projected gap `0`, and projection Postgres deadlocks `0`. This is useful promotion evidence for read-model freshness, but it is not the same claim as the `10k` venue-core materializer baseline.
...
- Bot Arena shared-time local hardening now has multi-instrument simulation proof, but not yet paced wall-clock proof. Local 2026-07-09 evidence in [`BOT_ARENA_DO_SIMULATION_SOAK_CHECKLIST.md`](./BOT_ARENA_DO_SIMULATION_SOAK_CHECKLIST.md): `local-live-3m-responsive-house-unpaced` covered `5` symbols, `18` bots, `6480` ticks/wakes, `1812` submitted venue commands, all terminal `COMPLETED`, projection drain passed, market-health passed, and rendered report/export completed. Remaining arena gap: add controlled taker/fill pressure and open-order/cancel-budget enforcement, then prove a real paced `3-5m` wall-clock gate before promoting to the `15m` DigitalOcean run.

## Current Forward Path

This is a short summary of the single active execution ladder in [`WORK_PLAN.md`](./WORK_PLAN.md#active-execution-ladder), not a second planning source. Work should follow that ladder unless a new decision supersedes it. The active checkpoint has moved from proving the direct materializer `10k` `soak-5m` gate to proving projection/read-model freshness under the same durable venue-event load while continuing API/control-plane hardening.
...
2. Promote projection/read-model freshness under durable venue-event load: enable the relevant projector/read surfaces, measure projected throughput and lag separately from canonical materialization, and keep `venue-core` command acceptance from being redefined by UI/control-room freshness.
...
10. Keep Bot Arena Phase 1 moving without overlapping the real-time stress dashboard work: local arena run evidence, persisted positive/negative gates, static operator reports, shared-time multi-instrument hardening, then paced local and hosted/backbone gate planning.
