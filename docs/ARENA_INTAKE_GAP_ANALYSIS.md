...
6. **Result ingestion gate** — partially closed, not Tier B (`BOT_ARENA_PLAN.md:88`): the local admin ingestion path now writes hosted-summary-shaped payloads into `arena.run_bot_results`, retries replace by `(runId, botId, versionId, scoringPolicyVersion)`, and `make dev-smoke-arena-run-results` proves readback plus leaderboard visibility. Still open: feed it a real hosted bot test summary once score calculation policy is accepted.
7. **Postgres schema drift risk** — explicit remaining-work item, not Tier B (`BOT_ARENA_PLAN.md:85`): keep `PostgresArenaBotRegistryStore` validation in schema-placement CI as arena schema evolves. Current coverage includes the full `arena.run_bot_results` column contract. Ongoing hygiene.
...
2. **Feed result ingestion from a real hosted bot summary** (#6) — concrete, scoped, no Tier B dependency once score calculation policy is accepted.
3. **Keep Postgres schema-placement CI coverage current** (#7) — concrete, scoped, no Tier B dependency.
