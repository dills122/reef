...
| `/api/v1/arena/leaderboard` | bot/user | `arena.run_bot_results + arena.bots` | durable public game read | durable rows written after run completion | none beyond arena result persistence completeness | public game data | active |
...
| `/api/v1/settlement/exceptions/{scenarioRunId}` | user/admin/test harness | settlement exception projection over append-only facts | projection-backed read | rebuildable queue over clearing rejections and settlement breaks | none beyond settlement fact persistence completeness | scenario settlement evidence | active |
