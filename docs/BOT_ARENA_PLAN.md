...
- implementation: `.github/workflows/bot-submission.yml` runs the unprivileged PR-side stages. Stages 1-3 are real (manifest validation, then `bot-sdk-test-bot.mjs` for the combined security-scan/sandboxed-test-run gate); they may use an ephemeral `make dev-up` stack since they need no persistent state. Dependabot PRs skip every job in that workflow. Stage 4 runs in `.github/workflows/bot-submission-provision.yml` as a trusted `workflow_run` handoff after the PR workflow succeeds on a `bots/add/`, `bots/update/`, or `bots/remove/` branch. Normal PR workflows do not enqueue the provisioning handoff. Instead, `.github/workflows/bot-submission-non-bot-status.yml` uses trusted base-branch metadata only to publish the shared `registry-diff-and-provision` status when a PR does not change `bots/**`; Dependabot receives an explicit not-applicable success from this direct path. It never checks out or executes PR code and has no hosted secret or OIDC access. The provisioning workflow checks out base-branch scripts, receives the hosted Admin API secret and GitHub OIDC permission, and calls `scripts/dev/bot-submission-provision-openbao.mjs`.
...
- outstanding manual step: GitHub branch protection on `master` must require
  ...
  `registry-diff-and-provision` context is published against the PR head SHA by
  the trusted `Bot Submission Provision` `workflow_run` for bot changes and by
  the metadata-only `Bot Submission Non-bot Status` workflow for all other
  changes, so branch protection should require that context rather than relying
  on either workflow's job name.
...
Merge gate for bot submissions: branch protection requires both all automated stages (1-4) passing AND at least one human operator approval before merge — automated checks alone do not auto-merge bot submissions. Dependabot PRs may use GitHub's native auto-merge after every branch-protection requirement passes; that automation is actor-scoped to `dependabot[bot]`, does not bypass requirements, and does not apply to bot submissions. This matches the operator-decision-driven pattern already used for lifecycle transitions (`ArenaOperatorDecision` records actor, reason, correlation ID for state changes elsewhere in the control plane); the PR reviewer approval is the equivalent operator checkpoint for the submission path.
...
- **Admin API** — done, as `platform-runtime` in the backbone deployment Compose stack. Same instance handles Arena registry/control-plane and bot-submission OpenBao provisioning; it is not a separate process.
- **Reverse proxy** — done. Caddy is defined in the backbone deployment Compose stack and gated behind `profiles: [public]`.
