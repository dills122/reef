...
- raw `/internal/*` routes stay private. GitHub Actions reaches only narrowly-scoped, bearer-token-gated admin gateway routes through Caddy: `/admin/v1/arena/bots` and `/admin/v1/arena/bots/openbao-provision`. Runtime binds the admin actor from request headers/token context, not body/query fields.
