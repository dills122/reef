...
- implementation: `.github/workflows/bot-submission.yml` runs the unprivileged PR-side stages. Stages 1-3 are real (manifest validation, then `bot-sdk-test-bot.mjs` for the combined security-scan/sandboxed-test-run gate); they may use an ephemeral `make dev-up` stack since they need no persistent state. Stage 4 runs in `.github/workflows/bot-submission-provision.yml` as a trusted `workflow_run` handoff after the PR workflow succeeds on a `bots/add/`, `bots/update/`, or `bots/remove/` branch. Normal PR workflows do not enqueue the provisioning handoff. Instead, `.github/workflows/bot-submission-non-bot-status.yml` uses trusted base-branch metadata only to publish the shared `registry-diff-and-provision` status when a PR does not change `bots/**`; it never checks out or executes PR code and has no hosted secret or OIDC access. The provisioning workflow checks out base-branch scripts, receives the hosted Admin API secret and GitHub OIDC permission, and calls `scripts/dev/bot-submission-provision-openbao.mjs`.
