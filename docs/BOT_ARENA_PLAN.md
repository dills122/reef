...
- raw `/internal/*` routes stay private. GitHub Actions and the admin UI reach only narrowly-scoped, authenticated admin gateway routes through Caddy: `/admin/v1/arena/bots`, `/admin/v1/arena/bots/openbao-provision`, `/admin/v1/arena/runs`, `/admin/v1/arena/run-bot-results`, `/admin/v1/arena/run-enforcement-events`, and `/admin/v1/arena/leaderboard`. Runtime binds the admin actor from request headers/token context, not body/query fields.
