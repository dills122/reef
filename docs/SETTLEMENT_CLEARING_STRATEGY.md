...
- The bounded post-trade sprint is [`POST_TRADE_LIFECYCLE_SPRINT.md`](./POST_TRADE_LIFECYCLE_SPRINT.md). Clearing/novation facts, exception queue v1, operator-readable lifecycle state, and focused tests are implemented on `master`. Remaining work is live security-fail/repair and `ops-realistic-v1` pending evidence plus a separately scoped operator-workflow follow-up. Any read-only netting seed must stay small and explicit. This work must not expand into full CCP/CNS clearing, production custody, or complete netted obligation settlement.
