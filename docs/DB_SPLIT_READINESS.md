...
- `command_log/0015_command_results_archive.sql` adds a `completed_at` range-partitioned archive target for terminal results; `PostgresCommandLogStore.archiveTerminalResults(...)` moves bounded unpinned live-result batches while preserving exact status lookup/accounting.
