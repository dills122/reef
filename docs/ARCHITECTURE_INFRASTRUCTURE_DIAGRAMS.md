...
- Normalized orders, executions, trades, runtime events, UI views, metrics, and leaderboards remain downstream projections. Projection has a separate recorded `2.5k/5m` sustained baseline. Historical `5k/60s` count/freshness passes do not close the August 21 downstream drain failure or promote sustained `5k`; projection write cost is separate from the `10k` venue-core claim.
