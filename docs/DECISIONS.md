...
- captured-ack submit workers batch runtime outcome persistence by preparing submit outcomes for the claimed batch, persisting them in a single transaction, and only then marking command-log terminal rows. This preserves the no-complete-before-persist ordering while reducing connection/transaction churn. Local evidence improved completed-during-load throughput but remains below target, so deeper runtime write batching or architecture changes are still needed.
