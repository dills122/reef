...
- The `stream-direct-nodb` stack profile already set `STREAM_ACK_INMEMORY_INTAKE_MAX_ENTRIES=100000` and `STREAM_ACK_INMEMORY_INTAKE_SHARDS=256`, but the local Compose runtime environment did not pass those values into platform-runtime containers. Runtime default `0` meant unlimited in-memory retention.
