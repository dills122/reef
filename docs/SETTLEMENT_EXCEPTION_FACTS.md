...
Historical note: the assertion surface above was the complete P2-era API surface. It is no longer the complete settlement API surface. Real ledger mutation has since shipped, and the runtime now also exposes `GET /api/v1/settlement/obligations/{scenarioRunId}` (obligation state projected from facts), `GET /api/v1/settlement/ledger/{scenarioRunId}` (replayable participant/account/asset balances plus per-settlement ledger proof totals), and `GET /api/v1/settlement/exceptions/{scenarioRunId}` (operator-readable clearing rejection and settlement break queue), alongside authenticated `/admin/v1/settlement/...` mutation routes for materialization and operator repair actions. See [`SETTLEMENT_CLEARING_STRATEGY.md`](./SETTLEMENT_CLEARING_STRATEGY.md#obligation-materialization) for the current, complete API surface and route list.
