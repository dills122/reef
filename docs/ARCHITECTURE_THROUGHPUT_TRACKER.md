...
- captured-ack submit workers now use one schema-owned `runtime.runtime_persist_submit_outcomes(jsonb)` call per prepared batch instead of repeated app-side single-outcome procedure calls. The function delegates to the existing single-outcome routine so canonical inserts, idempotency behavior, trace sequence allocation, and no-complete-before-persist ordering stay unchanged.
...
- conclusion: the bulk procedure reduces app-to-DB round trips and materially improves post-load drain in the smoke profile, but it does not reach the `7500` completed/sec target. Remaining measured blockers are still command-log append latency on intake and runtime persistence batch cost under worker contention. The next implementation slice should either collapse more runtime batch work into set-based SQL or reduce command-log append write amplification before tuning worker counts again.
