...
- Tightening plan before DigitalOcean/OpenTofu infra: finish crash/redelivery tests around async marker repair and canonical append idempotency; add projector rebuild/replay evidence from canonical facts; add drain-side lag/backpressure gates for worker/projector freshness; then start the DO droplet IaC harness and compare local evidence against expected hardware.
