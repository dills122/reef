...
class TradeSettlementObligationMaterializerTest {
    ...
    fun materializesTradeObligationsWithScenarioRunProfileEvidence() {
        ...
        assertEquals(1, result.materializedClearingSubmissions)
        assertEquals(1, result.materializedClearingAcceptances)
        assertEquals(0, result.materializedClearingRejections)
        assertEquals(1, result.materializedNovations)
        ...
        assertEquals(
            "settlement-clearing-submission-settlement-obligation-trade-1",
            facts.clearingSubmissions.single().settlementClearingSubmissionId
        )
        assertEquals(facts.affirmations.single().settlementAffirmationId, facts.clearingSubmissions.single().causationId)
        assertEquals(
            "settlement-clearing-acceptance-settlement-obligation-trade-1",
            facts.clearingAcceptances.single().settlementClearingAcceptanceId
        )
        assertEquals(facts.clearingSubmissions.single().settlementClearingSubmissionId, facts.clearingAcceptances.single().causationId)
        assertEquals("settlement-novation-settlement-obligation-trade-1", facts.novations.single().settlementNovationId)
        assertEquals(facts.clearingAcceptances.single().settlementClearingAcceptanceId, facts.novations.single().causationId)
        assertEquals("settlement-instruction-settlement-obligation-trade-1-1", facts.instructions.single().settlementInstructionId)
        assertEquals(facts.novations.single().settlementNovationId, facts.instructions.single().causationId)
    ...
    fun fallsBackToVenueSessionProfileWhenScenarioRunProfileIsAbsent() {
        ...
        assertEquals(1, facts.clearingSubmissions.size)
        assertEquals(1, facts.clearingAcceptances.size)
        assertEquals(1, facts.novations.size)
    ...
    fun realisticDefaultMaterializesObligationsWithoutStartingAttempts() {
        ...
        assertEquals(0, result.materializedClearingSubmissions)
        assertEquals(0, result.materializedClearingAcceptances)
        assertEquals(0, result.materializedClearingRejections)
        assertEquals(0, result.materializedNovations)
        ...
        assertEquals(emptyList(), facts.clearingSubmissions)
        assertEquals(emptyList(), facts.clearingAcceptances)
        assertEquals(emptyList(), facts.clearingRejections)
        assertEquals(emptyList(), facts.novations)
